# Official Links

---
date: 2026-05-27
status: Public link reference
source_baseline: Orina public project profile
---

## Web

| Channel | Link |
| --- | --- |
| Website | https://www.orina.io/ |
| Runtime app | https://app.orina.io/ |
| Blog | https://orina.io/blog |
| Whitepaper | https://whitepaper.orina.io/ |

## Community

| Channel | Link |
| --- | --- |
| X / Twitter | https://x.com/Orina_official |
| Discord | https://discord.gg/vythc6X9qF |
| Telegram official | https://t.me/orinaofficial |
| Telegram global | https://t.me/orinaglobal |
| Email | info@orina.io |

## Token Reference

| Field | Value |
| --- | --- |
| Token name | ORINA |
| Symbol | ORI |
| Chain | BNB Smart Chain |
| Standard | BEP-20 |
| Contract | [`0x093969C2Bb194e7424534918ECa5119FA72a61d6`](https://bscscan.com/token/0x093969c2bb194e7424534918eca5119fa72a61d6) |

## Repository Notes

This page records official public links for the documentation repository. It should not include private admin panels, unpublished channels, invite-only infrastructure, or internal operator contacts.
