...
The current public runtime reference is ATP v3.5 beta on BSC Testnet by default, with Base Sepolia, Arbitrum Sepolia, Ethereum Sepolia, Optimism Sepolia, and Avalanche Fuji also deployed as write-enabled testnet targets. The primary EIP-712 order domain still uses `MarketplaceATP` version `3.4` for signature compatibility, and the public runtime app is documented in [Runtime App](../runtime/live-app.md).
